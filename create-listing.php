...
width: 100%;
font-size: 0.95rem;
height: 46px; /* Altura fija para todos los inputs */
}
...
/* Estilo para el contenedor del input date */
...
/* Estilo para el input-group-text que precede al input date */
.input-group-prepend .input-group-text {
    background-color: #f1f3f9;
    border: 1px solid #e4e6ef;
    border-right: none;
...
/* Estilo para el icono dentro del input-group-text */
.input-group-prepend .input-group-text i {
    font-size: 0.9rem;
    color: var(--primary);
}

/* Estilo para el placeholder del input date */
            ...
            <div class="form-group">
                ...
                <div class="precio-container">
                    <div class="input-group precio-venta">
                        <div class="input-group-prepend">
                            <span class="input-group-text simbolo-moneda">$</span>
                        </div>
                        <input type="text" class="form-control" name="price" required min="0" step="0.01" placeholder="Ej: 4500.00">
                    </div>
        ...
        <div class="step-content" id="step2">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="section-card">
                        <div class="section-title">
                            <i class="fas fa-dollar-sign mr-2 p-3"></i>Métricas Financieras
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
    ...
    <div class="card shadow mb-4">
            ...
            <div class="form-group">
                ...
                <div class="custom-control custom-checkbox mb-2">
                    ...
                    <label class="custom-control-label" for="adsense">
                        <i class="fab fa-google mr-2 px-3"></i>Google AdSense
                    </label>
                </div>
                <div class="custom-control custom-checkbox mb-2">
                    ...
                    <label class="custom-control-label" for="sponsorships">
                        <i class="fas fa-handshake mr-2 px-3"></i>Patrocinios
                    </label>
                </div>
                <div class="custom-control custom-checkbox mb-2">
                    ...
                    <label class="custom-control-label" for="affiliate">
                        <i class="fas fa-link mr-2 px-3"></i>Afiliados
                    </label>
                </div>
                <div class="custom-control custom-checkbox mb-2">
                    ...
                    <label class="custom-control-label" for="products">
                        <i class="fas fa-shopping-cart mr-2 px-3"></i>Productos
                    </label>
                </div>
                <div class="custom-control custom-checkbox mb-2">
                    ...
                    <label class="custom-control-label" for="subscriptions">
                        <i class="fas fa-sync mr-2 px-3"></i>Suscripciones
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="step-content" id="step4" style="display: none;">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary">
                        <i class="fas fa-cog mr-2 p-3"></i>Configuración del Anuncio
                    </h6>
                </div>
                <div class="card-body">
                    <div class="row">
...
setupPriceFormatting: function() {
    ...
    updateMultipleValidation: function(maxMultiple) {
        ...
        if (profit > revenue) {
            ...
            profitInput.closest('.input-group').after('<div class="feedback-message invalid-feedback">El beneficio mensual no puede ser mayor que los ingresos mensuales</div>');
            $('input[name="revenue_multiple"], input[name="profit_multiple"]').val('');
            return false;
        }
        ...
        if (price <= 0 || profit <= 0 || revenue <= 0) {
            if (price <= 0) {
                ...
                priceInput.closest('.input-group.precio-venta').after('<div class="feedback-message invalid-feedback" style="display: block; margin-top: 0.5rem;">El precio debe ser mayor que cero</div>');
            }
            if (revenue <= 0) {
                revenueInput.removeClass('is-valid').addClass('is-invalid');
                revenueInput.closest('.input-group').after('<div class="feedback-message invalid-feedback">Los ingresos deben ser mayores que cero</div>');
            }
            if (profit <= 0) {
                profitInput.removeClass('is-valid').addClass('is-invalid');
                profitInput.closest('.input-group').after('<div class="feedback-message invalid-feedback">El beneficio debe ser mayor que cero</div>');
            }
            $('input[name="revenue_multiple"], input[name="profit_multiple"]').val('');
            return false;
        }
    ...
    showCurrentStep: function() {
        ...
        // Agregar la actualización del progress bar
        this.updateProgressBar();
        
        // Animación suave al cambiar de paso
        $('html, body').animate({
            scrollTop: $('.form-progress-container').offset().top - 20
        }, 500);
    },
    ...
    updateProgressBar: function() {
        ...
        // Actualizar la línea de progreso con animación
        $('.progress-line-active').css('width', `${progress}%`);
        
        // Actualizar los indicadores de paso
        $('.step-indicator').each((index, step) => {
        ...
        this.updateProgressBarA11y();
    },

    // Mejorar la accesibilidad
    updateProgressBarA11y: function() {
        $('.step-indicator').each((index, step) => {
            const stepNum = index + 1;
            const $step = $(step);
            const status = stepNum < this.currentStep ? 'completado' : 
                          stepNum === this.currentStep ? 'actual' : 
                          'pendiente';
            
            $step.attr('aria-label', `Paso ${stepNum}: ${status}`)
                 .attr('aria-current', stepNum === this.currentStep ? 'step' : null);
        });
    }
};
